...
import { DmViewReply, DmSegMobileReply, DmSegMobileReq, DmColorfulType } from "../protobuf/bilibili/community/service/dm/v1/dm.js";
			...
			switch (url.hostname) {
				...
				case "app.biliapi.com":
					switch (url.pathname) {
						...
						case "/x/v2/feed/index": // 推荐页
							switch (Settings?.Feed?.AD) {
								...
								default: {
									...
									let itemsCache = asArray(Storage.getItem("@BiliBili.Index.Caches", []));
									let singleItem;
									if (itemsCache.length > 0) {
										...
										itemsCache = asArray(Storage.getItem("@BiliBili.Index.Caches", []));
											if (itemsCache.length > 0) {
												singleItem = itemsCache.pop();
												Console.info("✅ 推荐页空缺位填充成功");
											}
						...
						case "/x/v2/feed/index/relate/story": // 首页短视频关联流
							...
							switch (Settings?.Feed?.Story) {
								...
								default:
									if (Array.isArray(body?.data?.items)) {
										...
										body.data.items = body.data.items
											.filter(item => isObject(item) && !isFeedAd(item) && !filterSet.has(item.card_goto))
											.map(item => cleanStoryItem(item, removeStoryCommercial, removeStoryTracking));
									}
									break;
								case false:
									Console.warn("用户设置首页短视频流广告不去除");
									if (Array.isArray(body?.data?.items)) body.data.items.forEach(item => cleanStoryItem(item, removeStoryCommercial, removeStoryTracking));
									break;
							}
							break;
				...
				case "api.live.bilibili.com":
					switch (url.pathname) {
						case "/xlive/app-interface/v2/index/feed": { // 直播首页推荐
						...
						case "/xlive/app-interface/v2/room/recList": { // 直播间推荐
							const removeTracking = Settings?.Privacy?.Tracking === true || Settings?.Privacy?.Strict === true;
							const removeCallbacks = Settings?.Xlive?.RemoveTrackingCallbacks === true || Settings?.Privacy?.Strict === true;
							const removePreloadTracking = Settings?.Xlive?.RemovePreloadTracking === true;
							if (Array.isArray(body?.data?.list)) {
								...
								if (removeTracking || removeCallbacks || removePreloadTracking) {
									body.data.list.forEach(card => cleanLiveCardTracking(card, removeTracking, removeCallbacks, removePreloadTracking));
								}
							}
							break;
						}
